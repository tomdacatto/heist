...
class Stalk(commands.Cog):
    def __init__(self, client):
        ...
        self.stalk_pool = redis.ConnectionPool(
            host='localhost',
            port=6379,
            db=1,
        ...
        self.stalk_redis = redis.Redis(connection_pool=self.stalk_pool)
        ...
        for stalker_id in stalkers:
            try:
                ...
                if activity_type == 'message':
                    ...
                    last_joined_channel = await self.stalk_redis.get(f"vc_join:{user_id}")

                    if last_joined_channel != channel_id:
                        description = f"**{username}** joined the voice channel **#{channel_name}** (<#{channel_id}>) in **{guild_name}**"
                        title = f"{username} - joined vc"
                        tasks.append(self.send_stalker_dm(stalker_id, title, description, user_pfp))

                        await self.stalk_redis.set(f"vc_join:{user_id}", channel_id, ex=3) # added small exp so it dont spam ts ts ts mANGO
    ...
    async def cog_unload(self):
        await self.stalk_pool.disconnect()
