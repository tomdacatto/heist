...
async def is_blacklisted(ctx):
    if isinstance(ctx, discord.Interaction):
        ...
        async with get_db_connection() as conn:
            result = await conn.fetchrow('SELECT user_id, information FROM blacklist WHERE user_id = $1', user_id)
            is_blacklisted = bool(result)
            await redis_client.setex(f"blacklist:{user_id}", 300, str(is_blacklisted))
            if is_blacklisted:
                await redis_client.setex(f"blacklist_reason:{user_id}", 300, result['information'])
                await ctx.response.send_message(
                    f"You are blacklisted from using [Heist](<https://{footer}>) for **\"{result['information']}\"**.\n-# Wrongfully blacklisted? You may appeal [**here**](<https://discord.gg/6ScZFN3wPA>).",
                    ephemeral=True
                )
            return not is_blacklisted
...
async def is_donor(interaction):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get("http://127.0.0.1:4323/api/commands/premium", timeout=5) as resp:
                if resp.status == 200:
                    data = await resp.json()
                    group_id = data.get("id")
                    mention = f"</premium perks:{group_id}>" if group_id else "`/premium perks`"
                else:
                    mention = "`/premium perks`"
        except Exception:
            mention = "`/premium perks`"
    if cached_result is not None:
        is_donor = cached_result == "True"
        if not is_donor:
            color = await get_embed_color(str(interaction.user.id))
            e = discord.Embed(description=f"<:1000081363:1437545821036875917> This is a premium-only command. Run {mention} to learn more.", color=color)
