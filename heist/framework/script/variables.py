...
def to_dict(
    ...
    log = logging.getLogger("heist/system")

    if isinstance(block, (Member, User)):
        log.debug(
            f"Processing {key} data: id={block.id}, name={block.name}"
        )
        data.update({
        ...
        if isinstance(block, Member):
            log.debug(
                f"Processing member data: guild_id={block.guild.id}, roles={[r.id for r in block.roles] if hasattr(block, 'roles') else 'N/A'}"
            )
            guild_avatar = (
            ...
            try:
                ...
                log.debug(
                    f"Member join position: {position}"
                )
                data.update({
                ...
                log.debug(
                    "Member not found in guild members list"
                )
                data.update({
                    "user.join_position": "N/A",
                    "user.join_position_suffix": "N/A",
                })
...
def parse(
    ...
    **kwargs,
    ...
    for block in local_blocks:
        if isinstance(block, tuple):
            ...
            if isinstance(value, (str, int, float)):
                ...
                variables.update(to_dict(value, key))
        else:
    ...
    def replace(match: re.Match) -> str:
        ...
        value = variables.get(name_mod) or variables.get(name)
