...
if TYPE_CHECKING:
    from heist.framework import heist, Context

TMP_ROOT = AsyncPath("/tmp")
CACHE_ROOT = TMP_ROOT / "heist"
...
class ConfirmView(OriginalView):
    ctx: Context
    ...
    async def interaction_check(
        ...
        if self.member and interaction.user != self.member:
            await interaction.embed(
                "You are not authorized to interact with this embed!"
            )
            return False
