...
from heist.shared.config import Authentication
from .discord import StrictRole, TouchableMember
from ..converters.regex import MEDIA_URL_PATTERN, DURATION_PATTERN
...
class Status(Converter[bool]):
    """
    A converter that converts a string to a boolean.
    """
    async def convert(
...
class StrictUser(UserConverter):
    """
    A converter that converts a string to a user.
    """
    async def convert(
        self, ctx: Context, argument: str
    ) -> User:
...
class StrictMember(MemberConverter):
    """
    A converter that converts a string to a member.
    """
    async def convert(
        self, ctx: Context, argument: str
    ) -> Member:
...
class Duration(Converter[timedelta]):
    """
    A converter that converts a string to a duration.
    """
    def __init__(
        self: "Duration",
    ...
    async def convert(
        self: "Duration", ctx: Context, argument: str
    ) -> timedelta:
        """
        A method that converts a string to a duration.
        """
        if not (
            matches := re.fullmatch(
                DURATION_PATTERN, argument, re.IGNORECASE
            )
        ):
            raise CommandError(
                "The duration provided didn't pass validation!"
            )

        units: Dict[str, int] = {
            unit: int(amount)
            for unit, amount in matches.groupdict().items()
            if amount
        }
        for unit in units:
            if unit not in self.units:
                raise CommandError(
                    f"The unit `{unit}` is not valid for this command!"
                )
        ...
        try:
            ...
            raise CommandError(
                "The duration provided is too long!"
            ) from exc

        if self.min and duration < self.min:
            raise CommandError(
                "The duration provided is too short!"
            )
...
class PartialAttachment:
    """
    A class that represents a partial attachment.
    """
    url: str
    buffer: bytes
    filename: str
    ...
    def __str__(self) -> str:
        """
        A method that returns the filename.
        """
        return self.filename

    def is_image(self) -> bool:
        """
        A method that checks if the attachment is an image.
        """
        return (
    ...
    def is_video(self) -> bool:
        """
        A method that checks if the attachment is a video.
        """
        return (
    ...
    def is_audio(self) -> bool:
        """
        A method that checks if the attachment is an audio.
        """
        return (
    ...
    def is_gif(self) -> bool:
        """
        A method that checks if the attachment is a gif.
        """
        return (
    ...
    def is_archive(self) -> bool:
        """
        A method that checks if the attachment is an archive.
        """
        return (
    ...
    @staticmethod
    async def read(
        ctx: Context, url: URL | str
    ) -> tuple[bytes, str]:
        """
        A method that reads an attachment from a URL.
        """
    ...
    def get_attachment(
        ...
        """
        A method that gets the attachment from a message.
        """
    ...
    async def convert(
        ...
        """
        A method that converts a string to a partial attachment.
        """
        try:
...
class Timezone(Converter[str]):
    """
    A converter that converts a string to a timezone.
    """
    async def convert(
        self, ctx: Context, argument: str
    ) -> str:
        """
        A method that converts a string to a timezone.
        """
        response = await ctx.bot.session.get(
            URL.build(
                ...
                query={
                    "q": argument.lower(),
                    "key": Authentication().keys.weather,
                },
            ),
        )
        if not response.ok:
            raise CommandError(
                f"Timezone not found for **{argument}**!"
            )
...
class Timeframe:
    """
    A class that represents a timeframe.
    """
    period: Literal[
        "overall",
        "7day",
    ...
    def __str__(self) -> str:
        """
        A method that returns the timeframe.
        """
        if self.period == "7day":
            return "weekly"
    ...
    def current(self) -> str:
        """
        A property that returns the current period.
        """
        if self.period == "7day":
            return "week"
    ...
    async def convert(
        ...
        """
        A method that converts a string to a timeframe.
        """
        if argument in {
            "weekly",
            "week",
