...
class ValidPermission(Converter):
    async def convert(self, ctx: Context, argument: str):
        ...
        """
        valid_permissions = [
            p[0] for p in ctx.author.guild_permissions
        ]

        if not argument in valid_permissions:
            return await ctx.embed(
                "This is not a valid permission!\n"
                f"-# Please run `{ctx.clean_prefix}fakepermissions permissions` to check all available permissions!",
                message_type="warn",
            )
...
def has_permissions(**permissions):
    async def predicate(ctx: Context):
        ...
        author_permissions = [
            p[0]
            for p in ctx.author.guild_permissions
            if p[1]
        ]
        if any(
            p in author_permissions for p in permissions
        ):
            return True

        roles = ", ".join(
            str(r.id) for r in ctx.author.roles
        )
        query = f"""
            SELECT permission 
...
def is_donator():
    async def predicate(ctx: Context):
        ...
        raise CommandError(
            f"You must be a **donator** to use `{ctx.command.qualified_name}` - [**Discord Server**](https://discord.gg/evict)"
        )
